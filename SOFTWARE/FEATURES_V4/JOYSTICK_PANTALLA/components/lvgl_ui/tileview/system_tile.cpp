...
lv_obj_t *label_time;
lv_obj_t *label_date;
lv_obj_t *label_cylinder_pressure;              // Té el nom de cylinder pressure però ara és una dada del joystick
lv_obj_t *label_cylinder_position = nullptr;    
lv_obj_t *label_cylinder_load;                  // Té el nom de cylinder load però ara és una dada del joystick


void system_set_joystick_vx(int vx)
{
    if (label_cylinder_load)
        lv_label_set_text_fmt(label_cylinder_load, "Vx: %d", vx);
}

void system_set_joystick_vy(int vy)
{
    if (label_cylinder_pressure)
        lv_label_set_text_fmt(label_cylinder_pressure, "Vy: %d", vy);
}
...
void system_tile_init(lv_obj_t *parent)
{
    ...
    list_item = lv_list_add_btn(list, NULL, "Joystick Vx");
    label_cylinder_load = lv_label_create(list_item);
    lv_label_set_text(label_cylinder_load, "Vx: ---");
    //lv_label_set_text(label_cylinder_load, "--- Kg");

    list_item = lv_list_add_btn(list, NULL, "Joystick Vy");
    label_cylinder_pressure = lv_label_create(list_item);
    lv_label_set_text(label_cylinder_pressure, "Vy: ---");
