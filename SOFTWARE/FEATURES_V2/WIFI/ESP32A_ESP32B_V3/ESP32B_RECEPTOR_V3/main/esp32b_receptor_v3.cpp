// ============================================================================
// 📘 ESP32B Receptor V3
// 🔹 Funció: Es conecta a la WIFI generada per un altre ESP32 i li envia ordres
//    ordres UDP amb un jostick. Té un LED RGB per visualitzar l'ordre
...
#define DEADZONE 200 // Zona morta

static const char *TAG = "ESP32B_RECEPTOR_V3";
...
// 🔹 Crear socket UDP client per enviar ordres
    int sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
    if (sock < 0) {
        ESP_LOGE(TAG, "No s'ha pogut crear el socket UDP client: errno %d", errno);
    }

    struct sockaddr_in dest_addr;
    dest_addr.sin_family = AF_INET;
    dest_addr.sin_port = htons(PORT);
    dest_addr.sin_addr.s_addr = inet_addr("192.168.4.1"); // IP del receptor
    ...
    while (true) {
        ...
        // Enviar direcció per UDP
        if (sock >= 0) {
            int err = sendto(sock, dir.c_str(), dir.size(), 0, 
                             (struct sockaddr*)&dest_addr, dest_len);
            if (err < 0) {
                ESP_LOGE(TAG, "Error enviant missatge UDP: errno %d", errno);
            }
        }
